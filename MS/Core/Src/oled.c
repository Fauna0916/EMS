...
//show all the data
void OLED_SHOW_DATA(double gas, AhtSensonValue *aht, HumanState humanstate)
{
  char mes[50];
  OLED_NewFrame();
  
  sprintf(mes, "gas:%.2lfkom", gas);
  OLED_PrintString(OLED_X_1, OLED_Y_1, mes, &font16x16, OLED_COLOR_NORMAL);
  sprintf(mes, "humidity:%.1f", sensorValue.ahtHumi);
  OLED_PrintString(OLED_X_1, OLED_Y_16, mes, &font16x16, OLED_COLOR_NORMAL);
  sprintf(mes, "temperature:%.1f", sensorValue.ahtTemper);
  OLED_PrintString(OLED_X_1, OLED_Y_30, mes, &font16x16, OLED_COLOR_NORMAL);
  if (humanstate == isHuman)
  {
    sprintf(mes, "isHuman");
  }
  else
  ...
  OLED_PrintString(OLED_X_1, OLED_Y_45, mes, &font16x16, OLED_COLOR_NORMAL);
